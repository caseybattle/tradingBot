...
class BacktestEngine:
    ...
    def _precompute(self, df: pd.DataFrame):
        """Compute SuperTrend + RSI once for the full dataframe."""
    ...
    def run(self, df: pd.DataFrame) -> BacktestResult:
        ...
        direction, rsi = self._precompute(df)
        ...
        for i in range(MIN_WINDOW, len(df)):
            ...
            if not position:
                ...
                side = None
                if bullish_flip and curr_rsi < self.rsi_upper and self.funding_rate < 0.0005:
                    side = "long"
                elif bearish_flip and curr_rsi > self.rsi_lower and self.funding_rate > -0.0005:
                    side = "short"
